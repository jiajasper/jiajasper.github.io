...
func _on_g_mouse_entered():
	object_cursor.can_place = false
	self.can_remove = true
	object_cursor.show()


func _on_g_mouse_exited():
...
func _on_g_body_entered(body):
	...
	if Global.playing == true:	
		...
		if player.player_color == "Red":
			player.player_color = "Yellow"
		if player.player_color == "Blue":
			player.player_color = "Cyan"
		if player.player_color == "Magenta":
			player.player_color = "White"
	else:
		pass

func _on_g_body_exited(body):
